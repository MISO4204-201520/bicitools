    ...
    public RespuestaJson exportarReporteMetricasUsuario(String usuario, String fechaIni, String fechaFin, String archivo) {
        ...
        if (qresul.size() > 0) {
            ...
            try {
                ...
                documento.add(new Paragraph(" "));
                try {
                    Image foto = Image.getInstance("/Users/jhony/Documents/Uni Andes/Fabricas/Bicitools/bicitools/Bicitools/bicitools.jpg");
            ...
            if (qresul.size() > 0) {
                ...
                for (i = 0; i < qresul.size(); i++) {
                    ...
                    if (res.getCodigo() == 0) {
                        ...
                        rutaSalida.setRecorridos(obtenerRecoRuta(usuario, fechaIni, fechaFin, miRuta.getNombre(), documento));
                        documento.add(new Paragraph("---------------------"));
                        documento.add(new Paragraph(" "));
                        rta.add(rutaSalida);
                        res = ConstruyeRespuesta.construyeRespuestaOk();
