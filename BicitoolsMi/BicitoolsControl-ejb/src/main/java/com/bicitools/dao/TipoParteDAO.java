...
public class TipoParteDAO implements TipoParteDAOLocal {
    ...
    @Override
    public String obtenerNombreTipoParte(int idTipo)
    {
        String res;
        Query query = em.createNamedQuery("TipoParte.findByIdTipoParte");
        query.setParameter("idTipoParte", idTipo);
        ArrayList qresul = (ArrayList) query.getResultList();

        
        if (qresul.size() > 0) {
            TipoParte tipoParte = (TipoParte) qresul.get(0);
            res = tipoParte.getNombre();
        }
        else
            res = null;
